			...
			<view class="top" v-show="current === 0">
				<view class="filter">
					...
					<view class="type-btns">
						...
						<view :class="{ 'type-btn': true, 'btn-selected': selected0 == 2 }" @click="selected0 = 2">冻结</view>
						<view :class="{ 'type-btn': true, 'btn-selected': selected0 == 3 }" @click="selected0 = 3">入账</view>
					</view>
				</view>
				<text class="count">共{{ selected0 == 1 ? '出账' :(selected0==2?'冻结':'入账') }}{{ totalCount0 }}笔,累计积分</text>
				<text class="amount">{{ amount0 }}</text>
				<text class="compare">{{ '较上' + items[current].slice(0, 1) + (upOrDown0 >= 0 ? '增加' : '减少') + Math.abs(upOrDown0) + '积分' }}</text>
				<view class="chart">
					<text class="title">{{ selected0 == 1 ? '出账' :(selected0==2?'冻结':'入账') }}统计表</text>
			...
			<view class="top" v-show="current === 1">
				<view class="filter">
					...
					<view class="type-btns">
						...
						<view :class="{ 'type-btn': true, 'btn-selected': selected1 == 2 }" @click="selected1 = 2">冻结</view>
						<view :class="{ 'type-btn': true, 'btn-selected': selected1 == 3 }" @click="selected1 = 3">入账</view>
					</view>
				</view>
				<text class="count">共{{ selected1 == 1 ? '出账' :(selected1==2?'冻结':'入账') }}{{ totalCount1 }}笔,累计积分</text>
				<text class="amount">{{ amount1 }}</text>
				<text class="compare">{{ '较上' + items[current].slice(0, 1) + (upOrDown1 >= 0 ? '增加' : '减少') + Math.abs(upOrDown1) + '积分' }}</text>
				<view class="chart">
					<text class="title">{{ selected1 == 1 ? '出账' :(selected1==2?'冻结':'入账') }}统计表</text>
					<view class="chart-container">
						<view class="qiun-columns">
							<view class="qiun-charts">
								<canvas
			...
			<view class="top" v-show="current === 2">
				<view class="filter">
					...
					<view class="type-btns">
						...
						<view :class="{ 'type-btn': true, 'btn-selected': selected2 == 2 }" @click="selected2 = 2">冻结</view>
						<view :class="{ 'type-btn': true, 'btn-selected': selected2 == 3 }" @click="selected2 = 3">入账</view>
					</view>
				</view>
				<text class="count">共{{ selected2 == 1 ? '出账' :(selected2==2?'冻结':'入账') }}{{ totalCount2 }}笔,累计积分</text>
				<text class="amount">{{ amount2 }}</text>
				<text class="compare">{{ '较上' + items[current].slice(0, 1) + (upOrDown2 >= 0 ? '增加' : '减少') + Math.abs(upOrDown2) + '积分' }}</text>
				<view class="chart">
					<text class="title">{{ selected2 == 1 ? '出账' :(selected2==2?'冻结':'入账') }}统计表</text>
					<view class="chart-container">
						<view class="qiun-columns">
							<view class="qiun-charts">
								<canvas
...
export default {
  ...
  data () {
    return {
      ...
      // 上次滚动加载数据的时间,滚动加载锁，滚动加载间隔最多1000+毫秒执行一次加载
  ...
  onLoad (option) {
    ...
    // 1、生成并渲染画布 2、请求数据并更新画布   3、添加画布监听器
  ...
  methods: {
    ...
    dateTimeChange (value) {
      ...
      this.getEchartData(this.timeDefer(value, this.range), this.range, 0, this.current)
    },
    /**
		 * @description 获取后台数据
    ...
    getEchartData (queryTime, range, UpDown, tabIndex) {
      ...
      base.api(
        config.host + '/rest/getCommissionFlowReport',
        {
          queryTime: o.current == 2 ? new Date().getFullYear() + '' : queryTime,
          queryType: this['selected' + tabIndex], // Number 1 出账 ，2 冻结 ，3 入账
          // range: o.current == 2 ? new Date().getFullYear() - 2018 : range,//如果是年，只获取含2019年之后的
          range: o.current == 2 ? new Date().getFullYear() - 2018 : 31, // 如果是年，只获取含2019年之后的
          timeUpDown: UpDown
        },
        function (res) {
    ...
    touchEndA (e) {
      ...
      canvaColumnArr[o.current].showToolTip(e, {
        format: function (item, category) {
          ...
          // 差值获取同比增长
          // o['upOrDown'+o.current]=index>0?item.data.value-o['uchartData' + o.current].series[0].data[index-1].value:0;
